...
function ensureSettings() {
    ...
    settings.loraMin = DEFAULT_SETTINGS.loraMin;
    settings.loraMax = DEFAULT_SETTINGS.loraMax;
...
function buildUserPrompt(messageId, { imageReference = null } = {}) {
    ...
    const defaultsStr = numericProps.map(prop => {
        const defaultVal = getComfyPlaceholderDefault(prop.key, prop.default);
...
function normalizeCandidate(value) {
    ...
    for (const prop of numericProps) {
        ...
        if (rawValue !== undefined) {
            normalized[key] = clampNumber(rawValue, prop.min, prop.max, getComfyPlaceholderDefault(key, prop.default));
        } else {
            normalized[key] = getComfyPlaceholderDefault(key, prop.default);
        }
    }
...
async function rebuildCandidateFromImage(messageId, mediaIndex) {
    ...
    for (const prop of numericProps) {
        ...
        imageReference[prop.key] = clampNumber(rawVal, prop.min, prop.max, getComfyPlaceholderDefault(prop.key, prop.default));
    }
